...
namespace Seven
{
	...
	public:
		...
		void RemoveFirst(T removal, Compare<T> compare) = 0;
		...
		bool TryRemoveFirst(T removal, Compare<T> compare) = 0;
		...
		/// <param name="compare">The function to determine equality.</param>
		void RemoveAll(T removal, Compare<T> compare) = 0;
	...
	public:
		...
		/// <summary>Returns the number of items in the list.</summary>
		/// <remarks>Runtime: O(1).</remarks>
		int GetCount{ get{ return _count; } }
		...
		#pragma region Structure<T>

		/// <summary>Invokes a delegate for each entry in the data structure.</summary>
		/// <param name="function">The delegate to invoke on each item in the structure.</param>
		void Stepper(Step<T> function)
		{
		...
		/// <summary>Invokes a delegate for each entry in the data structure.</summary>
		/// <param name="function">The delegate to invoke on each item in the structure.</param>
		void Stepper(StepRef<T> function)
		{
			for (Node looper = this._head; looper != null; looper = looper.Next)
			{
				T temp = looper.Value;
		...
		StepStatus Stepper(StepBreak<T> function)
		{
			for (Node looper = this._head; looper != null; looper = looper.Next)
			{
		...
		/// <param name="function">The delegate to invoke on each item in the structure.</param>
		/// <returns>The resulting status of the iteration.</returns>
		StepStatus Stepper(StepRefBreak<T> function)
		{
			for (Node looper = this._head; looper != null; looper = looper.Next)
			{
				T temp = looper.Value;
	...
	/// <remarks>The runtimes of each public member are included in the "remarks" xml tags.</remarks>
	template<typename T>
	struct List_Array : List<T>
	{
	public:

		#pragma region List_Array<T>
		...
		void SetMinimumCapacity(int value)
		{
			if (value < 1)
