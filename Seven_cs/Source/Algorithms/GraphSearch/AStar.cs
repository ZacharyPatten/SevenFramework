	...
	public static class AStar<T, Math>
	{
			...
			Goal goal)
		{
			...
			function function = (T node, Node previous) =>
				{
					Math costFromStart = Arithmetic.Add<Math>(computed_costs.Get(previous), cost(previous.Value, node));
					return Arithmetic.Add<Math>(costFromStart, heuristic(node));
				};
				...
				else
				{
					neighbors(current.Value,
						(T neighbor) =>
						{
							...
							Math costValue = Arithmetic<Math>.Add(computed_costs.Get(current), cost(current.Value, neighbor));
