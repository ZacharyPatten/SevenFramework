...
namespace Seven.Structures
{
	...
	public class Graph_SetOmnitree<T> : Graph<T>
	{
		...
		#region Graph_SetOmnitree<T>

		#region field

		public Set_Hash<T> _nodes;
		public Omnitree_LinkedLinkedLists<Edge, T> _edges;
		...
		{
			this._edges = graph._edges.Clone() as Omnitree_LinkedLinkedLists<Edge, T>;
		...
		public Graph_SetOmnitree(Equate<T> equate, Compare<T> compare, Hash<T> hash, T min, T max, Omnitree.Average<T> average)
		{
			...
			Omnitree.Locate_Final<Edge, T> locationFunction = (Edge a) => { return Accessor.Get(new T[] { a.Start, a.End }); };
			this._edges = new Omnitree_LinkedLinkedLists<Edge, T>(
				new T[] { min, min },
				new T[] { max, max },
	...
	public class Graph_Map<T> : Graph<T>
	{
		...
		public Map_Linked<Map_Linked<bool, T>, T> _map;
		int _edges;
		...
		{
			this._edges = 0;
			this._map = new Map_Linked<Map_Linked<bool, T>, T>(equate, hash);
		}
